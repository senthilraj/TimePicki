(function($) {
  ...
  $.fn.timepicki = function(options) {
    ...
    return this.each(function() {
      var ele = $(this),
          ele_hei = ele.outerHeight() + 10,
          ele_par = $(this).parents('.time_pick'),
          top_arrow_button = (settings.increase_direction === 'down')
              ? '<div class=\'prev action-prev\'></div>'
              : '<div class=\'prev action-next\'></div>',
          bottom_arrow_button = (settings.increase_direction === 'down')
          ...
          new_ele = $(
              ...
              (settings.disable_keyboard_mobile ? 'readonly' : '') + '></div>' +
              bottom_arrow_button +
              '</div>' +
              '<div class=\'mins\'>' +
              top_arrow_button +
        ...
        function validate() {
          ...
          if (grandfatherDiv.hasClass('time')) { /// HOUR
            ...
            if (isValidNumber) {
              ...
              // clip number.
              var hours = (settings.show_meridian) ? Math.min(
                  Math.max(parseInt(input.val()), 1), 12) : // for 12 hour date picker.
      ...
      inputs.on('keydown', function(e) {
        ...
        if ($currentInput.closest('.timepicker_wrap .time').length) {
          change_time(null, direction);
        } else if ($currentInput.closest('.timepicker_wrap .mins').length) {
          change_mins(null, direction);
        } else if ($currentInput.closest('.timepicker_wrap .meridian').length &&
      ...
      inputs.on('blur', function() {
        setTimeout(function() {
          ...
          if ($focusedElement.is(':input') &&
              !is_element_in_timepicki($focusedElement)) {
            set_value();
            close_timepicki();
          }
        }, 0);
      ...
      function is_element_in_timepicki(jquery_element) {
        return $.contains(ele_par[0], jquery_element[0]) ||
      ...
      function change_time(cur_ele, direction) {
        var cur_cli = 'time',
            ...
            step_size = Number(settings.step_size_hours);
      ...
      function change_mins(cur_ele, direction) {
        var cur_cli = 'mins',
            ...
            ele_st = 0,
            ele_en = 59,
            step_size = Number(settings.step_size_minutes);
      ...
      var cur_next = ele_next.find('.action-next'),
          cur_prev = ele_next.find('.action-prev');

      $(cur_prev).add(cur_next).on('click', function() {
        var currentElement = $(this);

        if (currentElement.parent().hasClass('time')) {
          change_time(currentElement);
        } else if (currentElement.parent().hasClass('mins')) {
          change_mins(currentElement);
        } else {
          if (settings.show_meridian) {
            change_meri(currentElement);
          }
        }
      });
